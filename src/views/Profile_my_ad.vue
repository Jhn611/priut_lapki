...
<template>
  <div class="background_profile_right"> 
    <div class="status_sobes" v-if="!load && armored_cats.length != 0"> <p> Мои объявления </p> </div>
      ...
      <div v-for="cat in armored_cats" :key="cat.id">
        <div class="profile-armored-cat">
          ...
          <div class="profile-armored-cat-right">
            ...
            <div class="profile-armored-cat-right-description"><p>Кота ещё не забронировали.</p> <button>Связаться</button></div>
          </div>          <div class="menu-modal" v-if="isModalOpen(cat.id)">
            <p class="menu-modal-option">Перейти к объявлению</p>
            <p class="menu-modal-option">Редактировать</p>
            <p class="menu-modal-option">Снять с публикации</p>
            <p class="menu-modal-option">Завершить</p>
          </div>
        </div>
      </div> 
    </div>
    <div class="status_sobes" v-if="!load && armored_cats.length == 0" @click="changeTab"> <p> Добавить котика </p> </div>
